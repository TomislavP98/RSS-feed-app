enum Strings {
    
    enum Localization {
        static let empty: String = ""
        static let addUrl: String = "Add URL"
        static let addApi: String = "Add API"
        static let like: String = "Like"
        static let dislike: String = "Dislike"
        static let delete: String = "Delete"
        static let addFeed: String = "Add feed"
        static let enteUrl: String = "Enter URL"
        static let enterApi: String = "Enter API key"
        static let allFeeds: String = "All feeds"
        static let likedFeeds: String = "Liked Feeds"
        static let ok: String = "Ok"         // This one is almost pointless
        static let loaderMessage: String = "If it keeps spinning, go back and chek on API or URL"
        static let invalidUrl: String = "Invalid entry"
    }
    
    enum Indentifiers {
        static let feedCell: String = "FeedCell"
        static let feedItemCell: String = "FeedItemCell"
    }
    
    enum Key {
        static let apiKey: String = "apiKeySecret"
    }

    
}
